...
if !(_execute) then {
	...
	if (isserver) then {
		...
		//--- Register
		//[[_function,_args,true],"BIS_fnc_onPlayerConnected",true,true] call life_fnc_mp;
		[_function,_args,true] remoteexec ["BIS_fnc_onPlayerConnected"];
	...
	//--- Execute
	//[[player,_args],_function,false,false] call life_fnc_mp;
	[player,_args] remoteexec [_function,2];
	true
};