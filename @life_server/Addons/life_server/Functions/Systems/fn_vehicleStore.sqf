...
if(_impound) then
{
	...
	{
		_query = format["UPDATE vehicles SET active='0', fuel='%3', damage='%4' WHERE pid='%1' AND plate='%2'",_uid,_plate, fuel _veh, _damage];
	...
	else
{
	...
	_query = format["UPDATE vehicles SET active='0', fuel='%1', damage='%4' WHERE pid='%2' AND plate='%3'",fuel _vehicle,_uid,_plate,_damage];
	waitUntil {!DB_Async_Active};
	_thread = [_query,1] call DB_fnc_asyncCall;
	//waitUntil {scriptDone _thread};
	if(!isNil "_vehicle" && {!isNull _vehicle}) then {
