...
if (_param isEqualTo "init") exitWith {
	...
	if ((floor random 99) == 0) exitWith {
		player setDamage 0;
	[10, "Se le ha ido la cabeza...", [1,0,0], 1.5, format ["...paco ha matado a %1 y se ha meado en su cara.", name player], [1,1,1], 1] remoteexec ["ica_fnc_anuncio",-2];
	};
	
	if (!isNil {_cagada}) then {
		hint format ["¡La has cagado! \nLa policía está en camino."];
		[10, "Aviso", [1,0,0], 1.5, "Están atracando la gasolinera de paco", [1,1,1], 1] remoteexec ["ica_fnc_anuncio",WEST];
