...
if (_param isEqualTo "roboDia") exitWith {
	...
	//if (_Policias < 10)exitWith{hint "Tienen que estar al menos 10 policias conectados, sino, no es divertido."};
	if (call mafia <= 0) exitWith {hint "Necesitas estar en una mafia o grupo terrorista para poder robar esto"};
	...
	[32,joyeria,1] remoteExec ["ica_fnc_robodesconexion",2];
	robosactivos = robosactivos + 3;
	...
	{robos_bloqueados = true; publicVariable "robos_bloqueados";sleep 1800;robos_bloqueados = nil; publicVariable "robos_bloqueados";} remoteExec ["BIS_fnc_spawn",2];
...
if (_param isEqualTo "serverDia") exitWith {
	...
	["generar"] spawn ica_fnc_diamantes;
	sleep 180;
	["vaciar"] call ica_fnc_diamantes;
	joyeria animate ["frontdoor1", 0];
	joyeria animate ["frontdoor2", 0];
};
