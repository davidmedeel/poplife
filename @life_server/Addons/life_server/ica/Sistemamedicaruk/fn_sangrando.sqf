...
if (_parte isEqualTo "loop") exitWith { // lo he cargado desde initMed
	while {true} do {
		...
		private "_txt";
		_txt = switch (true) do {
			case (_sangradoTotal >= 7): {"\Rex_Fotos\icons\medICA\sangrado4.paa"};
			case (_sangradoTotal >= 5): {"\Rex_Fotos\icons\medICA\sangrado3.paa"};
			case (_sangradoTotal >= 3): {"\Rex_Fotos\icons\medICA\sangrado2.paa"};
			case (_sangradoTotal >= 1): {"\Rex_Fotos\icons\medICA\sangrado1.paa"};
