	...
	switch (true) do {
		case ((animationState player) == "deadstate"): {
			llamadaDeSocorro = {
				...
				["recibo", player, format ["Mensaje automático: estoy herido en %1", mapGridPosition player]] remoteexec ["ica_fnc_mensajesica",independent];
		...
		case (side player == WEST): {
			...
			';
			emergensia = {
				if (!isNil {llamadaSocorroHecha}) exitWith {hint "Sólo puedes hacer una llamada de auxilio cada minuto";};
				0 spawn {llamadaSocorroHecha = true; sleep 60; llamadaSocorroHecha = nil};
				["recibo", player, format ["Aviso de Emergencia; El agente NIL solicita refuerzos en las coordenada: %2",mapGridPosition player]] remoteExec ["ica_fnc_mensajesica",civilian];
				titleText ["La centralita ha recibido tu llamada de emergencia", "PLAIN"];
			};
		};
	};
	_ruta = "icons\menuY\";
