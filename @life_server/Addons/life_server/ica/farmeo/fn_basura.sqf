...
if (_param isEqualTo "cargo") exitWith {
	...
	_veh = nearestObject [player, "stv_garbagetruck"];
	_contenedor = nearestObject [player, "stv_contenedordebasura"];
	...
	if ((_veh getvariable "basura") > 3) exitwith {hint "El camión está lleno"};
...
if (_param isEqualTo "descargo") exitWith {
	...
	_swain = (_veh getvariable "basura") * 3000;
	format ["%1 - (%2) ha cobrado %5 por recoger la basura - Dinero en banco : %3€ - Dinero en mano %4€", profileName,
	(getPlayerUID player),
	pop_atmdin,
	pop_din,
	_swain] remoteExecCall ["A3Log", 2];
...
if (_param isEqualTo "solicito") exitWith {
	...
	if (pop_din < 15000) exitwith {hint "La fianza del camión son 15000 €"};
	pop_din = pop_din - 15000;
	_veh = "stv_garbagetruck" createvehicle position player;
	_veh setvariable ["basura",0,true];
