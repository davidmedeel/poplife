	...
	if (_acierto >= _objetivo) then {
		...
		switch (ganzuando) do {
			case "veh": {
				...
				[getPlayerUID player,profileName,"487"] remoteexec ["life_fnc_wantedAdd",2];
			};
			case "cas": {
				_veh = param3;
				life_vehicles pushBack _veh;
				...
				[10, "Securitas Direct Informa", [1,0,0], 1.5, "Su casa esta siendo robada", [1,1,1], 1] remoteexec ["ica_fnc_anuncio",_uidpropietario];
				...
				[10, "Securitas Direct Informa", [1,0,0], 1.5, format["La casa de %1 esta siendo robada",_nombrepropietario]] remoteexec ["ica_fnc_anuncio",west];
			};
			case "concesionario": {
				ganzuando = nil;
