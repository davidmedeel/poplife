		...
		if ((["pop_uranio"] call ica_fnc_tengo >= 1) OR (["pop_uranio", true] call ica_fnc_tengo >= 1) OR (!isNil {radiactividad})) then { // loop largo buscando uranio o radiactividad
			...
			while {true} do { // loop corto hasta que deje de tener el item
				...
				if (radiactividad >= 14) then {
					...
					hint "Te sientes muy mareado y con naúseas";
					sleep 16;
...
if (_param isEqualTo "bomba") exitWith {
	...
	[10, "Aviso", [1,0,0], 1.5, "Explosión detectada en la central nuclear", [1,1,1], 1] remoteExec ["ica_fnc_anuncio",west, 2];
...
if (_param isEqualTo "extraer") exitWith {
	...
	hint "Tengo uranio encima, creo que esto no es muy bueno... Mejor se lo llevo a mi contacto Mohammed, seguro que él sabe que hacer con esto...";
...
if (_param isEqualTo "enriquecer") exitWith {
	...
	if ((player distance _tio) > 5) exitWith {
		hint "Esperar amigo, si tu no querer acercar a mi yo entender, pero que sepa que yo no enviar uranio 235 por correo. Pir cierto, hueles mal.";
	};	
	
	if !(["pop_uranio"] call ica_fnc_tengo > 0) exitWith {hint "No se que pretender tú amigo... Yo no compro nada ilegal... Por cierto... ¿Durum o Kebab?"};
	...
	hint "No preocupar amigo, yo entender mucho del tema ¿ah? Yo tardar 30 segundos";
	sleep 20;
	hint "Ya está casi istar amigo, yo profesional... *dice mientras se le cae algo al suelo*";
	sleep 10;
	hint "¡Ya está! Yo aumentar la proporción del isótopo U235 que me has traído del 0.71% al 3,4%... Yo prifisional!, pir cierto, tu no mover mucho el isótopo";
	
	if ((player distance _tio) > 5) exitWith {
		hint "Ya te dije que no enviaba nada por correo. Ah, por cierto, no se aceptan devoluciones.";
	};
	
	if (["pop_uranio"] call ica_fnc_tengo == 0) exitWith {hint "Pera amigo, ki ti pasah ah?, ¿tu querer liar a Ahmed?"};
...
if (_param isEqualTo "vender") exitWith {
	...
	if ((player distance _tio) > 5) exitWith {
		hint "No poder hacer business con usted si estar lejos ¿entender o no entender? ¿mi explico o no me explico?.";
	};	
	...
	hint "*acento árabe* Placer hacer business con usted, por cierto, ¿tú querer kebab pan de pita o querer pan normal?";
