...
if (_param isEqualTo "menu") exitWith {
	...
	{ if(player distance _x < 2) then {_cercanos pushBack _x};} foreach playableUnits;
	{
		if (!isNull _x AND alive _x AND player distance _x < 2 AND _x != player) then {
			_desplegable lbAdd format ["??? - %2", side _x];
...
if (_param isEqualTo "doy") exitWith {
	...
	format ["%1 - (%2) ha dado %3€ a %6 - (%7)  - Dinero en banco : %4€ - Dinero en mano %5€",	 profileName,
	(getPlayerUID player),
	_can,
	pop_atmdin,
	pop_din,
	name _quien,
	getPlayerUID _quien] remoteExecCall ["A3Log", 2];
