		...
		while {true} do {
			if (_t <= 0) exitWith {
				if (stance player == "STAND") exitWith {
					...
					[player,"AwopPercMstpSgthWrflDnon_start1"] remoteexec ["life_fnc_animSync",nil];
					sleep 1.5;
				...
				if (stance player == "CROUCH") exitWith {
					...
					[player,"AwopPknlMstpSgthWrflDnon_start"] remoteexec ["life_fnc_animSync",nil];
					sleep 1.5;
				...
				if (stance player == "PRONE") exitWith {
					...
					[player,"AwopPpneMstpSgthWrflDnon_start"] remoteexec ["life_fnc_animSync",nil];
					sleep 1.5;
