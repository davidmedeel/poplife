...
if ((life_inv_cb == 0) && (medel_din < _basePrice)) exitWith {hint "no tiene suficiente dinero en efectivo y no tiene tarjeta de credito !"};
...
if ((_gangbank >= _prixcb) && (life_inv_cb > 0)) then {
		...
		if (_action) then {
				...
				[0, format ["El vehiculo %1 fue comprado por %3 por el Precio %2$ Con el dinero de la Mafia", getText(configFile >> "CfgVehicles" >> _className >> "displayName"), [_prixcb] call life_fnc_numberText, name player]] remoteexec ["life_fnc_broadcast",_owner];
				...
				if (_action) then {
						...
						if (_basePrice > medel_din) exitWith {hint "no tiene suficiente dinero en efectivo !"};
						if (medel_din >= _basePrice) then {
...
if (_secure == 1) then {
		...
		if ((life_veh_shop select 0) == "med_air_hs") then {	// PENDIENTE
				...
				[_vehicle,_colorIndex] remoteexec ["life_fnc_colorVehicle"];
		...
		[getPlayerUID player,player,_vehicle,1] remoteexec ["TON_fnc_keyManagement",2];
		if (_mode) then {
