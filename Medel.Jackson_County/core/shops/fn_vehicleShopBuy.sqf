...
if(medel_din < _basePrice) exitWith {hint format[localize "STR_Shop_Veh_NotEnough",[_basePrice - medel_din] call life_fnc_numberText];};
...
if((life_veh_shop select 0) == "med_air_hs") then {
	...
	[_vehicle] call life_fnc_clearVehicleAmmo;
	//[[_vehicle,"vehicle_info_owners",[[getPlayerUID player,profileName]],true],"TON_fnc_setObjVar",false,false] call life_fnc_MP;
	[_vehicle,"vehicle_info_owners",[[getPlayerUID player,profileName]],true] remoteexec ["TON_fnc_setObjVar"];
	_vehicle disableTIEquipment true; //No Thermals.. They're cheap but addictive.
};
...
[getPlayerUID player,playerSide,_vehicle,1] remoteexec ["TON_fnc_keyManagement"];
...
if(_mode) then {
	if(!(_className in ["B_G_Offroad_01_armed_F","B_MRAP_01_hmg_F"])) then {
		...
		[(getPlayerUID player),playerSide,_vehicle,_colorIndex] remoteexec ["TON_fnc_vehicleCreate"];
	};
};
