...
if((_gangbank >= _prixcb) && (["pop_tarjeta"] call ica_fnc_tengo > 0)) then {
    ...
    "Mi dinero"
        ...
        _owner = (grpPlayer getVariable "gang_owner");
			...
			[0,format["Un arma %1 fue comprado por %3 por %2$ Con el dinero de la mafia",_itemInfo select 1,[_prixcb] call life_fnc_numberText,name player]] remoteexec ["life_fnc_broadcast",_owner];
