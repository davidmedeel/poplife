...
switch (_code) do {
	...
	//Map Key
	case _mapKey: {
		if (player getVariable "PoliON") then {
			if(!visibleMap) then {[] spawn life_fnc_copMarkers;};
		};
		if  (player getVariable "EMSON") then {
			if(!visibleMap) then {[] spawn ica_fnc_emsMarkers;};
		};
		if (!(player getVariable "PoliON") OR (player getVariable "EMSON")) then {
			if (!visibleMap) then {[] spawn life_fnc_civMarkers;};
		};
	};
	...
	//Interaction key (default is Left Windows, can be mapped via Controls -> Custom -> User Action 10)
	case DIK_LWIN:	{
		if ((player getVariable "PoliON") OR (player getVariable "EMSON")) then {[] call ica_fnc_abrirpuertas;};
	...
	case 63: {
		/*if (vehicle player == player && !(player getVariable ["restrained", false]) && !life_istazed) then	{
				if (player getVariable ["surrender", false]) then {
					...
					if (player getvariable "esposado") exitwith {};
	...
	case DIK_O: {
		if (!_shift && !_alt && !_ctrlKey && (player getVariable "PoliON") && (vehicle player != player)) then {
			...
			if(((typeof _veh) in _listaventa) && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
						...
						hint composeText [ image "\medel_fotos\icons\unlock.paa", "Vehículo abierto" ];
						//[[player,"abrecoche"],"life_fnc_say3D"] call life_fnc_MP;
						[player,"abrecoche"] remoteexec ["life_fnc_say3D"];
						...
						hint composeText [ image "\medel_fotos\icons\lock.paa", "Vehículo cerrado" ];
						//[[player,"cierracoche"],"life_fnc_say3D"] call life_fnc_MP;
						[player,"cierracoche"] remoteexec ["life_fnc_say3D"];
					};
				};
			};
		};
 ...
 //ANTI ALT + F4
	...
	/*	if(_alt && !_shift && !_ctrlKey) then {
			medel_atmdin = medel_atmdin * 0.85;
			_player setDamage 1;
...
/*	case DIK_DELETE: {
		if(_ctrlKey && _alt)  then {
			medel_atmdin = medel_atmdin - (medel_atmdin * 0.15);
...
/*	// ANTI CTRL + ESC
	...
	//F2 DEBUG Admin
	case DIK_F2: {
		if(primaryWeapon player == "cl_fishing_rod") then {
	...
	case 41 : {
		switch (side player) do
		{
			...
			case civilian: {
				if (player getVariable "PoliON") exitWith {[] call life_fnc_showBadge;}; [] call life_fnc_showDNI;
