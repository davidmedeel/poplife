	...
	if(toLower(_functionName) == "spy_fnc_cookiejar") exitWith {
		...
		if(_reportUID != _callerUID) exitWith {
			if(isServer && _mode == 0) then {
				...
				[_callerName,"False reporting to SpyGlass (cheater)"] remoteexec ["SPY_fnc_notifyAdmins"];
			};
			_exitScope = true;
		};
	};
...
_ban = ["bis_fnc_endmission","endmission","failmission","playmission","forceend","startloadingscreen"];
if (toLower (_functionName) in _ban) exitWith { // ica
	//[format ["Cheat global INTERCEPTADO ----- caller: %1 (%2)", _callerName, _callerUID], "diag_log", false, false, false] call BIS_fnc_MP;
	[format ["Cheat global INTERCEPTADO ----- caller: %1 (%2)", _callerName, _callerUID]] remoteexec ["diag_log",2];
	false
};
