...
while {true} do {
	if (animationState player != "CL3_anim_managerdoorlockpick") then {
		...
		[player,"CL3_anim_managerdoorlockpick"] remoteexec ["life_fnc_animSync"];
...
//	[[getPlayerUID player,profileName,"487"],"life_fnc_wantedAdd",false,false] call life_fnc_MP;
	...
	[getPlayerUID player,profileName,"215"] remoteexec ["life_fnc_wantedAdd",2];
	//[[0,"STR_ISTR_Lock_FailedNOTF",true,[profileName]],"life_fnc_broadcast",west,false] call life_fnc_MP;
	[0,"STR_ISTR_Lock_FailedNOTF",true,[profileName]] remoteexec ["life_fnc_broadcast",-2];
