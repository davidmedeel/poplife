...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship") || (_vehicle isKindOf "Motorcycle") || (_vehicle isKindOf "A3L_Tahoe_Base")) then
{
	...
	[0,format[localize "STR_NOTF_BeingImpounded",(_vehicleData select 0) select 1,_vehicleName]] remoteexec ["life_fnc_broadcast"];
	...
	if((count crew _vehicle) == 0) then
	{
		...
		switch (true) do
		{
			case (_vehicle isKindOf "Car"): {if(player getVariable "PoliON") then {_price = (call life_impound_car_cop);}else{_price = (call life_impound_car);};};
		...
		[0,format[localize "STR_NOTF_HasImpounded",profileName,(_vehicleData select 0) select 1,_vehicleName]] remoteexec ["life_fnc_broadcast",2];
